...
void Huffman::create_node_array()
{
    ...
    // Read in the characters and caculate
    while( !in_file.eof() )
    {
        in_file.get(in_char);
        if( in_file.eof() )
    ...
    // insert pseudo-EOF with frequency of 1
    Node_ptr node = new Huffman_node();
    node->id = PSEUDO_EOF;
    node->freq = 1;
    node->code = "";
...
void Huffman::create_map_table(const Node_ptr node, bool left)
{
    ...
    // If it is a leaf node, it is a "valid" node and is added to the table
...
void Huffman::do_compress()
{
    ...
    // Write the file with the number of nodes (including pseudo EOF) + Huffman tree + Huffman code

    // Number of write nodes in line 1 (int)
    out_file << size << endl;

    // Line 2 ~ (size + 1) is written into Huffman tree, that is, each line is written with character + Huffman code, such as "43 00100"
    for(table_it = table.begin(); table_it != table.end(); ++table_it)
    {
        out_file << table_it->first<< " "<< table_it->second<<endl;
    }

    // Write Huffman code in line size + 2
    in_file.clear();
    ...
    while( !in_file.eof() )
    {
        ...
        if( in_file.eof() )
            break;
        // Find the Huffman code corresponding to each character
        ...
        // When the total encoding length is greater than the preset write_ BUFF_ Size, write them to the file
        length = code.length();
        if(length > WRITE_BUFF_SIZE)
        {
            out_string.clear();
            //Write Huffman code to the output file as a binary stream
            for(i = 0; i + 7 < length; i += 8)
            {
                // Every 8 bit encoding is converted to an unsigned char output
                out_c = 0;
                for(j = 0; j < 8; j++)
                {
                    if('0' == code[i+j])
    ...
    // After reading all the files, insert pseudo-EOF 
    ...
    // Dealing with Huffman code remains
    length = code.length();
    out_c = 0;
    for(i = 0; i < length; i++)
    {
    ...
    for(i = 0; i < size; ++i)
    {
        ...
        for(j = 0; j < length; ++j)
        {
            ...
            if(tmp == NULL)
            {
                ...
                // If it is 0 or 1 at last, it means that it's a leaf node
                {
                    new_node->id = id;
                    new_node->code = code;
                }
            ...
            //If it is not null and there is already a node in the last bit of the Huffman code, there must be a problem
            else if(j == length -1)
            {
                printf("Huffman code is not valid, maybe the compressed file has been broken.\n");
                exit(1);
            }
            //If it is not empty, but the node is already a leaf node, there must be a problem too
